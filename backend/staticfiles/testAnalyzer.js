...
export class TestAnalyzer {
    ...
    calculateAverageFixationDuration() {
        if (!this.testData || this.testData.fixations.length === 0)
            return '0ms';
    ...
    calculateMaxFixationDuration() {
        if (!this.testData || this.testData.fixations.length === 0)
            return '0ms';
        const max = Math.max(...this.testData.fixations.map((f) => (f.endTime ?? 0) - f.startTime));
    ...
    calculateMinFixationDuration() {
        if (!this.testData || this.testData.fixations.length === 0)
            return '0ms';
        const min = Math.min(...this.testData.fixations.map((f) => (f.endTime ?? 0) - f.startTime));
     ...
     */
    getEyeStatus() {
        if (!this.testData || !this.testData.eyeStatus)
            return 'Non détecté';
        const { leftEyeOpen, rightEyeOpen } = this.testData.eyeStatus;
        if (leftEyeOpen && rightEyeOpen)
            return 'Les deux yeux ouverts';
        if (leftEyeOpen)
            return 'Œil gauche ouvert';
        if (rightEyeOpen)
            return 'Œil droit ouvert';
        return 'Yeux fermés';
    }
}
