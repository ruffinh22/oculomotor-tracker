...
export function renderNavbarContent() {
    ...
    return state.isAuthenticated
        ...
        <div class="flex items-center gap-4">
            ...
            <button class="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg text-sm font-medium transition-colors" onclick="app.logout()">
                Déconnexion
            </button>
        </div>
    `
                        ...
                        <div class="bg-white rounded-lg shadow-md p-8">
                            ...
                            <div class="space-y-4">
                                ...
                                <input type="password" id="loginPassword" placeholder="Mot de passe" 
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                <button class="w-full px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg transition-colors"
    ...
    return `
        <div class="max-w-6xl mx-auto">
            <div class="bg-white rounded-lg shadow-md p-8">
                ...
                <div id="testCanvas" class="bg-gray-50 rounded-lg border-2 border-gray-300 w-full h-96 flex items-center justify-center mb-8">
            ...
            .map((test, idx) => `
                                <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                                    <div class="flex justify-between items-start mb-2">
                                        ...
                                        <span class="px-3 py-1 rounded-full text-sm font-medium ${test.result === 'excellent'
            ...
            <div class="bg-white rounded-lg shadow-md p-8">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" id="registerUsername" placeholder="Votre nom d'utilisateur"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                        <input type="email" id="registerEmail" placeholder="votre.email@example.com"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" id="registerAge" placeholder="Votre âge" min="1" max="120"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    ...
                    <div>
                        ...
                        <input type="password" id="registerPasswordConfirm" placeholder="Confirmer votre mot de passe"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    
                    <button class="w-full px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg transition-colors"
                        onclick="window.app.handleRegister()">
                        S'inscrire
                    </button>
                </div>
...
function renderStatisticsScreen() {
    ...
    const excellentCount = tests.filter((t) => t.result === 'excellent').length;
    const goodCount = tests.filter((t) => t.result === 'good').length;
    const acceptableCount = tests.filter((t) => t.result === 'acceptable').length;
    const poorCount = tests.filter((t) => t.result === 'poor').length;
    const avgTracking = totalTests > 0 ? tests.reduce((sum, t) => sum + (t.trackingPercentage || 0), 0) / totalTests : 0;
    const avgStability = totalTests > 0 ? tests.reduce((sum, t) => sum + (t.gazeStability || 0), 0) / totalTests : 0;
    return `
...
export { renderHomeScreen, renderCalibrationScreen, renderTestScreen, renderResultsScreen, renderRegisterScreen, renderStatisticsScreen };
